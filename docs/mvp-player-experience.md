...
in onboarding labels, settings from pause, held-input suppression on resume,
...
Paused changes apply through a fresh input instance on resume. Release all old
sources; suppress keys/buttons already held at resume until released. Keys used
for binding must not become phantom hits. Test captures involving the existing

...
Status: remaining implementation plan, reconciled with commit `1b4b8f3` on 2026-09-12.
...
| ABI | Production WASM, kinds 1–34, generated bindings, compact advance, session/replay/sample bridge and coordinate export | Final render/static-resource and loop/voice contracts, reserve/admission integration |
...
| Presentation | Odin viewport transform, borrowed simulation projection, arena-backed active set and separate projection output | Component feedback history, animation, trail/follow points, HUD, meshes and draw batches |
...
| Evidence | Full engine suite, 38 browser service tests, four Chromium checks, 34 diagnostic/compact WASM schedules and 72 pinned component comparisons | Whole-scenario M2, H11/A12/A21/A22, complete browser/input/audio matrix and measured performance |
...
The prerequisite patch was verified and preserved in `061f19a`; compact output
and active projection were committed in `1b4b8f3` on `codex/m3-completion`.
Do not repeat the import. Inspect ancestry before bringing these changes into
another worktree, preserving any newer cleanup or implementation commits.
Keep the reconciled history containing `88b73ea`, `3062eee` and `cf379bc`.
The separate cleanup task may consolidate paths; use the responsibilities below
and reconcile against its reviewed result rather than resurrecting deleted files.
...
- Reuse `oe_session_advance_output` and kind 31, which omit diagnostic objects.
  Keep `oe_session_advance`/kind 19 for compatibility and diagnostics, outside
  the final frame path. Finish durable audio admission and the final render
  protocol; measure remaining input/sample scans separately. Preserve output
  token scope, retry behavior and invalidation rules.
...
- Extend the implemented borrowed simulation projection with component feedback
  and semantic cursor history needed by presentation. Keep its existing ownership
  and acyclic dependencies. Follow the package dependency table and update the
  relevant ADR for further ownership/contract changes.
...
1. Confirm the committed baseline and reconcile any subsequent cleanup changes;
   do not repeat the prerequisite import. Existing verification commands are `npm --prefix engine test`,
...
2. Finish W01's render/resource/audio contracts and reserve/validation code in
   `engine/abi/records.json`, `engine/runtime/`, `engine/audio_protocol/` and the
   ADR/interface documents. Reuse compact output, coordinate conversion, borrowed
   projection and active indices. Follow the detailed W01–W03 breakdown below.

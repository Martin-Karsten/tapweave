...
Status: all three phases implemented — A (replay watch from results + save
download), B (skip during lead-in and breaks, including in watch mode) and
C (decoder-owned song select metadata; see the song-select decoder metadata
section in `docs/status.md`). This document scopes the next product-shell

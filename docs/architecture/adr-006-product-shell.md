...
engine's pinned playfield framing handles aspect and letterboxing inside the
surface it is given. The layout avoids percentage-height chains below the min-height-only
...
[ADR-003](adr-003-rendering.md#pinned-lazer-playfield-framing). Leaving the route ends

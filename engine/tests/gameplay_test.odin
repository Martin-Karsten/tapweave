...
gameplay_note_lock_failure_and_replay_round_trip :: proc(test: ^testing.T) {
	...
	testing.expect_value(test, session.simulation.journal[0].cause, simulation.Cause.NOTE_LOCK)
	testing.expect_value(test, session.simulation.journal[0].object_id, 0)
	testing.expect_value(test, session.simulation.journal[1].result, core_types.Hit_Result.GREAT)

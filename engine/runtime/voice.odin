...
voice_reserve :: proc(instance: ^Instance, engine, session_handle: core_types.Handle, command_count: u64, arena_bytes: u64) -> core_types.Status {
	...
	required, count_status := voice_required_bytes(command_count)
	if count_status != .OK {
		return count_status
	}
...
write_voice_capacity :: proc(session: ^Session, requested, required: u32) {
	...
	required_bytes, _ := voice_required_bytes(u64(required))
...
oe_session_voice_reserve :: proc "c" (engine, session_handle: core_types.Handle, request_address, span_output: uintptr) -> u32 {
	...
	if get_u32(request, ABI_VOICE_RESERVE_FLAGS_OFFSET) != 0 || get_u64(request, ABI_VOICE_RESERVE_RESERVED_OFFSET) != 0 {
		return abi_status(.UNSUPPORTED)
	}
	requested := get_u32(request, ABI_VOICE_RESERVE_COMMAND_CAPACITY_OFFSET)
	required := u32(len(session.simulation.audio))
...
oe_session_voice_output :: proc "c" (engine, session_handle: core_types.Handle, span_output: uintptr) -> u32 {
	...
	if len(events) > int(session.voice_storage.capacity) {
		write_voice_capacity(session, session.voice_storage.capacity, u32(len(events)))
	...
	for event, event_index in events {
		write_voice_command(bytes[ABI_VOICE_FRAME_SIZE + event_index * ABI_VOICE_COMMAND_SIZE:], audio_protocol.one_shot_command(event))
	}
	...
	put_u32(bytes, ABI_VOICE_FRAME_FLAGS_OFFSET, 0)
	...
	put_relative_span(bytes, ABI_VOICE_FRAME_COMMANDS_OFFSET_OFFSET, ABI_VOICE_FRAME_SIZE, len(events), ABI_VOICE_COMMAND_SIZE)
	put_u32(bytes, ABI_VOICE_FRAME_RESERVED_OFFSET, 0)
	total_bytes := ABI_VOICE_FRAME_SIZE + len(events) * ABI_VOICE_COMMAND_SIZE
...
oe_transport_capabilities :: proc "c" (engine: core_types.Handle, span_output: uintptr) -> u32 {
	...
	put_u32(bytes, ABI_TRANSPORT_CAPABILITIES_VOICE_COMMAND_MASK_OFFSET, 1) // One-shot producer only.

...
score_rejection_failure_and_rounding :: proc(test: ^testing.T) {
	...
	for raw_score, rounding_index in ([]f64{0.5, 1.5, 2.5, 3.5}) {
		rounded, status := scoring.round_score(raw_score)
		testing.expect_value(test, status, core_types.Status.OK)
		testing.expect_value(test, rounded, expected_rounding[rounding_index])
	}
	for raw_score in ([]f64{-1, math.nan_f64(), math.inf_f64(1), 9_223_372_036_854_775_808.0}) {
		_, status := scoring.round_score(raw_score)

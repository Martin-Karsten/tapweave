...
const server = supplied_url ? null : spawn('npm', ['run', 'preview:hosting'], {
...
try {
  ...
  for (let attempt_index = 0; attempt_index < 60; attempt_index++) {
    if (server && server.exitCode !== null) throw new Error('Wrangler exited before becoming ready');
    try {
      const response = await fetch(base_url, { signal: AbortSignal.timeout(2000) });
      if (response.ok) { ready = true; break; }
    } catch { /* Wait for the local server or the published deployment. */ }
    await delay(1000);
  }
  assert.ok(ready, 'hosting endpoint did not become ready');
  for (const route of ['/', '/menu', '/select', '/play', '/results', '/diagnostics']) {
    const response = await fetch(new URL(route, base_url), {
      headers: { 'Sec-Fetch-Mode': 'navigate' }, redirect: 'error',
    });
  ...
  for (const asset_path of ['/assets/missing.js', '/assets/missing.wasm']) {
    const response = await fetch(new URL(asset_path, base_url), { headers: { 'Sec-Fetch-Mode': 'cors' } });
  ...
  const wasm_response_pending = page.waitForResponse(response => /\/assets\/tapweave-[\w-]+\.wasm$/.test(response.url()));
